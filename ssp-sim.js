...
var ssp_authn_txn = {
   ...
   print: function (){
      ...
      for (var keyId in keys) {
         ...
         if (Number(rec.endTime) !== 0) {
            ...
            var str1 = `ssp_authn{client=\"${rec.appName}\",source=\"${rec.source}\",credential1=\"${rec.primaryCredential}\",credential2=\"${rec.secondaryCredential}\",factor=\"${rec.riskFactor}\",result=\"${rec.result}\",txnId=\"${keys[keyId]}\"} ${duration}\n`
            if (debugLevel > 1) {console.log(str1);};
            ssp_authn_response += str1 ; 
            var str2 = `ssp_risk{client=\"${rec.appName}\",source=\"${rec.source}\",credential1=\"${rec.primaryCredential}\",credential2=\"${rec.secondaryCredential}\",factor=\"${rec.riskFactor}\",result=\"${rec.result}\",txnId=\"${keys[keyId]}\"} ${rec.riskScore}\n`
   ...
   start: function(key,timestamp) {
      ...
      if (trans == undefined) {
         ...
         ssp_authn_txn.txn.source = "SSP";
         ssp_authn_txn.txn.endTime = 0.0;
         ssp_authn_txn.txn.riskScore = 0;
         ssp_authn_txn.txn.result = "";
         ssp_authn_txn.txn.credential1 = "";
         ssp_authn_txn.txn.credential2 = "";
         ssp_authn_txn.txn.appName = "";
         ssp_authn_txn.txn.factor = "";
...
app.post('/sspLogStream', function(req, res) {
  ...
  for (var i = 0; i < req.body.length ; i++) {
     ...
     if (result != null) { 
        ssp_authn_txn.start(req.body[i].txnId, req.body[i].date); 
     }
